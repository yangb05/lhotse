...
def _read_raw_manifest(corpus_dir):
    ...
    """
    yodas_vi000_manifest, yodas_vi100_manifest, yodas_vi101_manifest = set(), set(), set()
    for subset in sorted(corpus_dir.iterdir()):
        if subset.stem == "yodas_vi000":
            target = yodas_vi000_manifest
        ...
        duration_info = {}
        # get wav info
        for wav in (subset / "untar").iterdir():
    ...
    return yodas_vi000_manifest, yodas_vi100_manifest, yodas_vi101_manifest
...
def prepare_yodas_vi(
    ...
    yodas_vi000_manifest, yodas_vi100_manifest, yodas_vi101_manifest = _read_raw_manifest(corpus_dir)
    prefix = "yodas"
    for split, raw_manifest in zip(
        ["vi000",],
        [yodas_vi000_manifest,],
    ):
...
if __name__ == "__main__":
    corpus_dir = Path("/data_a100/userhome/yangb/data/yodas/data")
    output_dir = Path("/data_a100/userhome/yangb/data/yodas/manifests")
    # prepare_yodas_vi(corpus_dir=corpus_dir, output_dir=output_dir, num_jobs=80)
